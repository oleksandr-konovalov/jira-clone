...
import { ProjectId, ProjectSearchData } from "@domain/project";
import { getProjectsSummaryWithIssues, deleteProject } from "@infrastructure/db/project";
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const projectsSearchData = await getProjectsSummaryWithIssues(userId);

  return json<LoaderData>({ projectsSearchData });
};
...
export default function ProjectsRoute() {
  const { projectsSearchData } = useLoaderData() as LoaderData;
  return <ProjectsView projectsSearchData={projectsSearchData} />;
}
